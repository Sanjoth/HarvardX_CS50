...
bool load(FILE* file, BYTE** content, size_t* length)
{
     // initialize buffer for chars
    char* buffer = NULL;

    // buffer capacity
    unsigned int capacity = 0;

    // to check chars in buffer
    unsigned int n = 0;

    // For reading chars
    int c;

    //  get chars from stdio
    while ((c = fgetc(file)) != EOF)
    {
        // grow buffer if necessary
        if (n + 1 > capacity)
        {
            // determine new capacity but start at 512
            if (capacity == 0)
            {
                capacity = 512;
            }
...
{
    // Get pointers to spaces
    char* space = strchr(line, ' ');
    int len1 = space - line;
    if (space == NULL || len1 == 0)
    {
        error(400);
        return false;
    }
    
    char* sp2 = strchr(space + 1, ' ');
    int len2 = sp2 - (space + 1);
    if (sp2 == NULL || len2 == 0)
    {
        error(400);
        return false;
    ...
    // Setting ouput HTML message with codes
    char* method = (char*) malloc(sizeof(char) * (len1 + 1));
    strncpy(method, line, len1);
    method[len1] = '\0';
    
    char* request_target = (char*) malloc(sizeof(char) * (len2 + 1));
    strncpy(request_target, space + 1, len2);
    request_target[len2] = '\0';
    
    char* HTTP_ver = (char*) malloc(sizeof(char) * (len3 + 1));
    strncpy(HTTP_ver, sp2 + 1, len3);
    HTTP_ver[len3] = '\0';
